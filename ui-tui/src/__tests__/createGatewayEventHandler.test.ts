...
describe('createGatewayEventHandler', () => {
  ...
  it('does not render or ACK a Kanban card for a different active session', () => {
    const appended: Msg[] = []
    const ctx = buildCtx(appended)
    const rpc = ctx.gateway.rpc as ReturnType<typeof vi.fn>

    patchUiState({ busy: false, sid: 'bot-chat' })
    createGatewayEventHandler(ctx)({
      type: 'kanban.notification',
      ...
      payload: {
        board: 'default',
        created_at: 1,
        delivery_key: 'chief:default:8',
        delivery_seq: 8,
        event_kind: 'completed',
        outbox_id: 43,
        task_id: 't_43',
        task_title: 'Wrong session'
      }
    })

...
def _build_section(home: Path) -> str:
    ...
    return (
        ...
        "behind a busy Bot Chat turn. When the user adds a requirement or asks whether "
        "an active/delivered artifact considered a new principle, reconcile it into the "
        "existing work graph: identify the exact task and artifact HEAD; append the "
        "acceptance criterion to the active card, or create a linked remediation child if "
        "the card is already done/reviewing; continue the same branch/PR where safe; then "
        "rerun independent review and final gates. Preserve the prior completion history. "
        "Never start duplicate implementation, queue a DM to the busy worker, or only "
        "describe the gaps in chat without persisting them to Kanban.\n"

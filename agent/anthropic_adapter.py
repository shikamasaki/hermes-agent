...
def resolve_claude_subscription_token() -> Optional[str]:
    ...
    This is intentionally narrower than :func:`resolve_anthropic_token` and
    never falls back to API-billed credentials or the shared credential pool.
    """
    ...
    if creds:
        if is_claude_code_token_valid(creds):
            ...
            # refresh token here without atomic Keychain write-back could
            # invalidate the authoritative entry.

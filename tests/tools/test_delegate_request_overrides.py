...
4. active adaptive route catalog.
...
from agent.delegation_routing import RouteDecision, RouteRequest

from tools.delegate_tool import (
    _merge_request_overrides,
    _resolve_delegation_credentials,
    _resolve_routed_credentials,
)
...
# ── Branch 4: active adaptive route catalog ───────────────────────────────


@patch("hermes_cli.runtime_provider.resolve_runtime_provider")
@patch("agent.delegation_routing.select_route")
def test_routed_branch_merges_explicit_over_runtime(mock_select, mock_resolve):
    """Adaptive native routes honor the same explicit-over-runtime contract."""
    runtime_overrides = {
        "service_tier": "default",
        "extra_body": {"thinking": {"type": "disabled"}, "clash": "runtime"},
    }
    explicit_overrides = {
        "service_tier": "priority",
        "extra_body": {"provider": {"sort": "throughput"}, "clash": "explicit"},
    }
    mock_select.return_value = RouteDecision(
        selected=True,
        reason="selected",
        route_id="route-a",
        provider="provider-a",
    ...
    catalog = SimpleNamespace(
        routes=[SimpleNamespace(id="route-a", is_claude_p=False)],
        usage_ttl_seconds=300,
        usage_stale_seconds=1800,
    )

    creds = _resolve_routed_credentials(
        {"request_overrides": explicit_overrides},
        catalog,
        RouteRequest(),
        route_context={"available_providers": {"provider-a"}, "usage_view": {}},
    )
    ...
    creds["request_overrides"]["extra_body"]["thinking"]["type"] = "mutated"
    assert runtime_overrides["extra_body"]["thinking"]["type"] == "disabled"
    assert explicit_overrides["extra_body"]["clash"] == "explicit"

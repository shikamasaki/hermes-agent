...
def test_failed_rotation_diagnostics_redact_exception_message(
    hermes_home, claude_credentials, monkeypatch, caplog
):
    """Logs and persisted quarantine status expose only the exception type."""
    monkeypatch.setattr(AA, "refresh_anthropic_oauth_pure", _rotating_refresh)
    real_replace = os.replace

    def _secret_bearing_failure(src, dst):
        if os.path.basename(os.fspath(dst)) in _SINGLETON_FILENAMES:
            raise OSError("account@example.com persist-secret")
        return real_replace(src, dst)

    monkeypatch.setattr(AA.os, "replace", _secret_bearing_failure)
    ...
    diagnostics = caplog.text + str(quarantined.last_error_message or "")
    assert "OSError" in diagnostics
    assert "account@example.com" not in diagnostics
    assert "persist-secret" not in diagnostics

    # The Hermes PKCE writer has the same redaction contract.
    oauth_file = hermes_home / ".anthropic_oauth.json"
    oauth_file.write_text("{}", encoding="utf-8")
    with caplog.at_level("ERROR"), pytest.raises(CredentialPersistError):
        AA._write_hermes_oauth_credentials("access", "refresh", 1)
    assert "account@example.com" not in caplog.text
    assert "persist-secret" not in caplog.text


def test_pool_refresh_diagnostics_redact_endpoint_exception(
    hermes_home, claude_credentials, monkeypatch, caplog
):
    """The generic pool catch must not log secret-bearing refresh exceptions."""

    def _secret_bearing_refresh(*_args, **_kwargs):
        raise RuntimeError("account@example.com cc-refresh-secret")

    monkeypatch.setattr(AA, "refresh_anthropic_oauth_pure", _secret_bearing_refresh)
    entry = _entry("claude_code")
    pool = CredentialPool("anthropic", [entry])
